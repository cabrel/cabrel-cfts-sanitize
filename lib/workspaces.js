...
internals.process = function(workspace, callback) {
  ...
  workspace.creator = userProcessor.deepProcess(workspace.creator);

  for (var i = 0; i < workspace.owners.length; i++) {
    workspace.owners[i] = userProcessor.deepProcess(workspace.owners[i]);
  }

  for (var i = 0; i < workspace.committers.length; i++) {
    workspace.committers[i] = userProcessor.deepProcess(workspace.committers[i]);
  }

  for (var i = 0; i < workspace.guests.length; i++) {
    workspace.guests[i] = userProcessor.deepProcess(workspace.guests[i]);
  }
